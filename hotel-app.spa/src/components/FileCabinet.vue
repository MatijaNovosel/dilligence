			...
			<v-list dense v-if="content != null" class="no-padding">
				<div class="text-center">
					<v-sheet tile :color="headerColor" class="py-3">
						<v-row justify="center">
							<span class="white--text">{{ content.naslov }}</span>
				...
				>
					<v-list-item-icon class="mt-4">
						<v-icon size="25">{{ fileIcon(file.naziv.slice(file.naziv.lastIndexOf(".") + 1)) }}</v-icon>
					</v-list-item-icon>
					<v-divider class="ml-n4 mr-4" vertical />
					<v-list-item-content>
						<v-list-item-title>{{ file.naziv }}</v-list-item-title>
					...
					<v-list-item-action>
						<v-row>
							...
							<v-btn icon text :loading="file.deleting" @click="deleteFile(file)">
